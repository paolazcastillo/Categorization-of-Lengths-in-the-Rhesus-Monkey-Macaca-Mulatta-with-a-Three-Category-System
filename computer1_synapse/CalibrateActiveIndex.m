...
% InitJoystickRelay.m's startup calibration, before
% CommunicationCategTaskACTX.m's shared reward/marker loop is running, so the
% stall is harmless there. StepJoystickRelay.m's PERIODIC recalibration does
% NOT call this function: it re-implements the same two-snapshot technique as
% a non-blocking state machine (first snapshot on one call, second snapshot
% on a LATER call once waitSec has actually elapsed) so recalibrating never
% freezes that shared loop. Calling this blocking version from there would
% freeze reward delivery for ~2*waitSec whenever a target hit landed during
% a recalibration window (observed 2026-07-30); see StepJoystickRelay.m's
