...
%       (colour<->category)
%     Analysis: TrialKinematics (per-trial speed/acceleration),
%       KalmanTrajectorySmoother, HampelFilter and ButterworthLowpass (the
%       trajectory-filtering stages it uses), NormInvNoTB
%       (toolbox-free normal quantile)
...
pixel_pitch        = bars.pixelPitch;    % also used for cm/px in the kinematics block
...
% exit never chose a category (chosen_target_color stays -1), so it isn't a
...
% what was on screen.
reaction_times_lenpos        = cell(numLengths, 4);
...
trial_error_type = zeros(1, maxAttempts);   % 0 = correct, 1 = early exit, 2 = wrong
...
blockStatsTemplate = struct('total', 0, 'good', 0, 'errorEarly', 0, 'errorWrong', 0, ...
    'totalGrp', zeros(1, 3), 'goodGrp', zeros(1, 3), 'nCat2', 0, 'nCat3', 0);
...
% an invitation to compare a px/s figure from one rig against a px/s figure
% from another, where the same number means a different real speed.
% Centimetres per second is the one physical unit that survives a change of
% monitor, resolution or viewing distance, so it is the only one logged.
% NumMovementSamples stays: it is not another copy
% of the same measurement but the QC signal for it, how many real samples
% backed the interpolation that produced these numbers (see TrialKinematics.m).
% It is always the true raw count, including on rows whose three kinematics
...
% with utilityScripts/measureJoystickNativeRate.m (median inter-change
% interval -> ~125 Hz native USB HID update rate, the common full-speed
% default). Polling narrower than that just re-reads the same cached HID
...
% TrialKinematics also enforces its own minDt floor as a second guard.
...
% TrialKinematics.m's resampling grid (interpGridDt) must match whichever
% input source actually fed this session's trajectory, not stay fixed at
% the joystick's rate, 'rz2adc' delivers CONSECUTIVE samples of the RZ2
% circuit's ~1017 Hz ADC (~0.98ms apart), forwarded at RELAY_HZ*N_READ =
% 1014 samples/s (kept just below the writer on purpose, see
% computer1_synapse/InitJoystickRelay.m), ~8x finer than the 'joystick'
% path's ~125 Hz. The grid follows the ADC rate, not the forwarding rate:
% the ~3 samples/s the relay skips appear as gaps in the sample index, not
% as wider spacing between the samples that do arrive. 'mouse' has no
% dedicated native rate of its own, so it falls back to the joystick grid
% (moveOversampleDt) too.
kinematicsGridDt = moveOversampleDt;
if useRZ2
    kinematicsGridDt = OrgGet(orgParams, 'rz2GridDt', 1 / 1017);
end

% Cutoff of the zero-phase Butterworth low-pass TrialKinematics.m runs on
% that grid before differentiating (see its STAGE 2 block). 20 Hz sits above
% the bandwidth of voluntary arm movement and well below Nyquist for both
% input sources, so the same value serves 'joystick' (125 Hz) and 'rz2adc'
% (1017 Hz) sessions; it is exposed here only so a session with an unusually
% noisy input can be tightened without editing the analysis file.
kinematicsCutoffHz = OrgGet(orgParams, 'kinematicsCutoffHz', 20);   % Hz; 0 disables the low-pass

% How TrialKinematics.m's STAGE 1 handles single-sample position glitches
% before anything is resampled. 'kalman' (default) is RTS smoothing alone;
% 'hampel' runs a median screen of the raw positions first. Neither uses the
% smoother's innovation gate: that gate rejects genuine samples through a
% real reach's peak curvature and extrapolates over them, which is what
% produces physically impossible peak velocities/accelerations on a subset
% of trials (see TrialKinematics.m's STAGE 1 block).
% hampelHalfWindow/hampelNSigma are that screen's window and threshold and
% are read regardless of method, so switching the popup mid-session-planning
% doesn't need them re-entered ('kalman' simply ignores them).
kinematicsOutlierMethod = OrgGet(orgParams, 'kinematicsOutlierMethod', 'kalman');   % 'kalman' | 'hampel'
hampelHalfWindow        = OrgGet(orgParams, 'hampelHalfWindow', 3);                 % samples per side ('hampel' only)
hampelNSigma            = OrgGet(orgParams, 'hampelNSigma', 3);                     % scaled-MADs ('hampel' only)
% QC floor: a MOVEMENT segment with fewer raw samples than this cannot
% resolve a velocity peak at the ~120 Hz effective logging rate, so its
% kinematics are reported as NaN rather than as an unreliable magnitude.
% NumMovementSamples in the CSV still carries the true raw count for every
% trial, so a NaN'd row stays auditable. kinematicsMinMoveDurSec is the same
% guard expressed in seconds and is off ([]) by default.
kinematicsMinMoveSamples = OrgGet(orgParams, 'kinematicsMinMoveSamples', 5);        % raw MOVEMENT samples
kinematicsMinMoveDurSec  = OrgGet(orgParams, 'kinematicsMinMoveDurSec', []);        % seconds; [] disables
...
while exitFlag == 0
    ...
    if setOnce_Trial
        ...
        % Timing definitions. The three INTERVALS, and their column order in
        % the CSV, match the earlier engine generation (centerOutTask_v2_2.m
    ...
    switch nextEpoch
                ...
                if inTarget(correctTarget)
                    if ~t.reachTarget
                        ...
                        if current_trial_color > 0 && current_trial_direction > 0
                            ...
                            % the CSV column of that name in the same session.
                            % Deliberately still decisionTime, so mean_RT_matrix
                            % stays comparable with every earlier generation of
                            % this report. What disambiguates it is the label in
                            % SessionReport.session, which prints the interval
                            % explicitly ("[target-onset -> leave-center]")
                            % instead of relying on the word "reaction".
                            reaction_times{current_trial_color, current_trial_direction}(end+1) = decisionTime;
                            ...
                            reaction_times_lenpos{lenIdxNow, current_trial_direction}(end+1) = decisionTime;
        ...
        case EP.ERROR_FB
            ...
            if ~t.marker
                ...
                % (error_type == 1), never a deliberate wrong-target pick: an
                % early exit is treated as not having genuinely attempted the
                % trial, so it should not be what finally closes it out. In
                % that case keep chaining fresh requeues indefinitely; only an
                ...
                if useRequeue && ~repeat_trial && (~isRequeuedSlot(trial_sequence_index) || error_type == 1)
...
try
    mean_RT_matrix = nan(3, 4);
    percent_correct_matrix = nan(3, 4);
    for i = 1:3
        for j = 1:4
            if ~isempty(reaction_times{i, j})
                mean_RT_matrix(i, j) = mean(reaction_times{i, j});
            end
    ...
    mean_RT_lenpos = nan(numLengths, 4);
    percent_correct_lenpos = nan(numLengths, 4);
    for i = 1:numLengths
        for j = 1:4
            if ~isempty(reaction_times_lenpos{i, j})
                mean_RT_lenpos(i, j) = mean(reaction_times_lenpos{i, j});
            end
    ...
    S.mean_RT_matrix = mean_RT_matrix;      S.percent_correct_matrix = percent_correct_matrix;
    ...
    S.reaction_times_lenpos = reaction_times_lenpos;
    S.total_trials_lenpos = total_trials_lenpos;
    S.correct_trials_lenpos = correct_trials_lenpos;
    S.mean_RT_lenpos = mean_RT_lenpos;      S.percent_correct_lenpos = percent_correct_lenpos;
    ...
    fprintf(fid, ['BarLengthIndex,BarSizeVA_deg,StimulusGroup,Direction,' ...
        'CorrectTrials,TotalTrials,PctCorrect_pct,MeanRT_s\n']);
    for li = 1:numLengths
        for di = 1:4
            fprintf(fid, '%d,%.4f,%s,%s,%d,%d,%.4f,%.4f\n', ...
                ...
                percent_correct_lenpos(li, di), mean_RT_lenpos(li, di));
        end
    end
    fclose(fid);
...
% Audible end-of-session alert, LAST: every device is released and every
% file written by this point, so a machine with no working audio can only
% cost the sound itself (AlertTaskDone swallows its own errors anyway).
