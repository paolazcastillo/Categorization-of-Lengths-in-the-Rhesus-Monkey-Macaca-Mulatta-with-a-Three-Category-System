...
try
    ...
    if isfield(rz2, 'clock') && ~isempty(rz2.clock)
        ...
        fprintf('RZ2 clock: %d observation(s) accepted, %d refused (queue not clear), %d fit(s) of which %d updated the slope, residual rms %.1f ms\n', ...
            c.nObservations, c.nObservationsRejected, c.nFits, c.nSlopeFits, c.residualRmsSec * 1000);
        if c.nSlopeFits == 0
            fprintf(['RZ2 clock: the slope never left the seed (window did not reach the ' ...
                'span needed); the estimated rate above IS the seed. Normal for a short ' ...
                'session.\n']);
        end
        ...
        if c.nSlewLimited > 0
            fprintf(['RZ2 clock: %d refit(s) were slew-limited. Normal while the link is ' ...
                'jittery; the map is absorbing arrival noise gradually instead of stepping.\n'], ...
                c.nSlewLimited);
        end
