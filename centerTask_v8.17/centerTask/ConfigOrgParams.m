classdef ConfigOrgParams
    ...
    methods (Static)
        function orgParams = getTaskDefaults()
            ...
            % Which output the end-of-session alert plays through. [] (default)
            % = MATLAB's sound(), i.e. the system default output. Set it to an
            % audiodevinfo OUTPUT device ID (the numbers testAlarmSpeakers.m
            % lists and plays through) to send the alert to a SPECIFIC device
            % instead -- for a rig whose speakers are not on the OS default
            % output. See AlertTaskDone.m.
            orgParams.alertAudioDevice = [];
            ...
            % the source of the physically impossible peak velocities /
            % accelerations that used to appear on a subset of trials. See
            % TrialKinematics.m's STAGE 1 block.
