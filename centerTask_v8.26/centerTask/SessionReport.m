...
%   Level 2  session          session-wide totals, quotas, timing/accuracy
...
% Depends on NormInvNoTB.m (toolbox-free normal quantile) for d'.
    ...
    methods (Static)
        ...
        fprintf('%-6s %-9s %6s %6s %8s %8s %8s %8s   %s\n', ...
            'Block', 'Mix(2/3)', 'Total', 'Good', '%Corr', 'EarlyEx', 'WrongTg', 'HoldBrk', ...
            'By category (good/total)');
        for b = 1:numel(blockStats)
            ...
            % errorHoldbreak is read with a guard so a blockStats struct
            % built by an older caller (without the field) still prints.
            if isfield(bs, 'errorHoldbreak'), hb = bs.errorHoldbreak; else, hb = 0; end
            fprintf('%-6d %2d/%-6d %6d %6d %7.1f%% %8d %8d %8d   %s\n', ...
                b, bs.nCat2, bs.nCat3, bs.total, bs.good, pct, bs.errorEarly, bs.errorWrong, hb, catStr);
        end
        fprintf('============================================\n');
        end
        ...
        % still works. error_holdbreak is optional for the same reason and is
        % APPENDED at the end rather than slotted next to the other two error
        % counts, so every existing positional argument keeps its index.
        if nargin < 16, sessionMode = ''; end
        if nargin < 22 || isempty(error_holdbreak), error_holdbreak = 0; end
        haveLenPos = nargin >= 21 && ~isempty(mean_decision_lenpos);
        if strcmpi(sessionMode, '2cat')
        ...
        if total_trials > 0
            fprintf('--- Error Breakdown (ErrorType in trial_data_*.csv) ---\n');
            fprintf('1 Early exit / timeout: %5d (%6.2f%%)  never reached the correct target\n', ...
                error_early_exit,   error_early_exit/total_trials*100);
            fprintf('2 Wrong target:         %5d (%6.2f%%)  entered a target of another category\n', ...
                error_wrong_target, error_wrong_target/total_trials*100);
            fprintf('3 Hold-break:           %5d (%6.2f%%)  reached the CORRECT target, released\n', ...
                error_holdbreak,    error_holdbreak/total_trials*100);
            % 41 = the exact prefix width of the three lines above (24-char
            % label + %5d + ' (' + %6.2f + '%)' + two spaces), so the wrapped
            % half of the hold-break description lines up under the first.
            fprintf('%41sbefore completing the hold (pre-training only)\n', '');
        end
        % Denominator is deliberately the trials that resolved as a CATEGORY
        % CHOICE: a correct trial or a wrong-target pick. Hold-breaks are
        % excluded even though the subject did touch a target -- the trial
        % never resolved into a choice, and folding a motor/patience failure
        % into a categorization accuracy would misreport exactly the thing
        % this line is meant to measure. They are reported on their own line
        % above instead.
        sel = good_trials + error_wrong_target;
        if sel > 0
            fprintf('\n--- Category Selection Performance ---\n');
            fprintf('Trials that resolved as a category choice: %d\n', sel);
            fprintf('Category selection accuracy: %.2f%%\n', good_trials/sel*100);
        end
        fprintf('=============================================\n\n');
        ...
        fprintf('\n--- Timing definitions (as written to trial_data_*.csv) ---\n');
        ...
        for g = activeCats
            fprintf('%s    %8.3f  %8.3f  %8.3f  %8.3f\n', rowName{g}, mean_decision_matrix(g, :));
        end
        fprintf('\nPercentage Correct (%%):\n%s\n', dirHeader);
        for g = activeCats
        ...
        nLengths = size(mean_decision_lenpos, 1);
        ...
        fprintf('--- Same, per (bar length x position) --- the %d combinations this session ran\n', ...
            nLengths * 4);
        fprintf('\nMean Decision Time (s) [target-onset -> leave-center]:\n%s\n', lenHeader);
        for L = 1:nLengths
            fprintf('%s%s\n', SessionReport.lenRowLabel(L, target_angles, lengthCategory, catNames), ...
                SessionReport.numRow(mean_decision_lenpos(L, :), '%8.3f'));
        end
        fprintf('\nPercentage Correct (%%):\n%s\n', lenHeader);
        for L = 1:nLengths
            fprintf('%s%s\n', SessionReport.lenRowLabel(L, target_angles, lengthCategory, catNames), ...
